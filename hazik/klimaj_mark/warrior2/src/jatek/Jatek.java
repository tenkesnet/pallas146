	...
	public static void main(String[] args) {
		...
		while (karakter.eletEro > 0.0 && karakter1.eletEro > 0.0) {
			...
			if (karakter1.eletEro < 0.0) {
				System.out.println("A " + karakter.nev + " a (" + karakter.className + ") nyert");
			}
			// javitando
			if (karakter1.eletEro < karakter1.eletEroMax / 2 && !karakter1.isBuffed() && karakter.eletEro>0.0) {
				karakter1.Buff();
				karakter1.sebzes = karakter1.Orvloves();
			} else if (karakter1.isBuffed()) {
				karakter1.sebzes = karakter1.Orvloves();
			}
			karakter.checkHP();
			karakter1.checkHP();
